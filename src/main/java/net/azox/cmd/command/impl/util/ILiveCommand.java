...
public final class ILiveCommand extends BaseCommand {
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            if (AzoxCmd.getInstance().getILiveManager().isEnabled(player)) {
                AzoxCmd.getInstance().getILiveManager().disableILive(player);
        ...
        AzoxCmd.getInstance().getILiveManager().enableILive(player, mode, damageReduction, foodHeal);
